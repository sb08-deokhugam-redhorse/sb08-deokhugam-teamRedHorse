...
public class AlarmController implements AlarmApi {
    ...
    public ResponseEntity<NotificationUpdateRequest> updateAlarmToRead(
            @Valid @PathVariable(value = "notificationId",required = true) UUID notificationId,
            @Valid @RequestParam(value = "Deokhugam-Request-Id",required = true) UUID deokhugamRequestUserID
    ){
