...
- 도서 등록/단건 조회/수정/논리 삭제/물리 삭제 엔드포인트 구현

- ISBN 중복 불가, 생성 후 수정 불가 비즈니스 규칙 적용

- `@SQLRestriction("is_deleted = false")`으로 논리 삭제 자동 필터링

- MapStruct로 Entity → DTO 변환 (`reviewCount` Long→int, `rating` null 처리)
...
- 커서 기반 페이지네이션: `cursor`(정렬값) + `after`(createdAt 타이브레이커)

- `limit+1` 전략으로 `hasNext` 감지

- 도서 단건 조회 Caffeine 캐싱 적용, 수정/삭제 시 `@CacheEvict`로 캐시 무효화
...
- ISBN으로 네이버 도서 정보 자동 조회 및 썸네일 Base64 인코딩

- Naver API 응답 Caffeine 캐싱 적용 (24시간)

- OCR Space API로 책 표지 이미지에서 ISBN 추출

- ISBN 파싱: 정규식 + OCR 오인식 보정 (O→0, l→1) + 체크섬 검증
...
- OCR Space ISBN 인식 실패 시 Textract 직접 호출 (서킷브레이커 미집계)

- Textract Queries 방식으로 ISBN 직접 추출
...
- 썸네일 S3 업로드 후 조회 시 presigned URL 자동 변환 (10분 만료)

- 매일 01:00 전날 로그 파일 S3 업로드

- 업로드 실패 시 `failed_uploads.txt`에 경로 저장 후 다음 스케줄러 실행 시 재시도
